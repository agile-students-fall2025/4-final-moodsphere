...
describe('GET /api/calendar', () => {
  ...
  it('should deduplicate dates from moods and entries on the same day', async () => {
    ...
    // Create mood and entry on same day
    await request(app)
      .post('/api/moods')
    ...
    expect(dateCount).to.equal(1); // Should only appear once despite two entries
  });
  ...
  it('should return dates in sorted order', async () => {
    ...
    expect(res.status).to.equal(200);
    // Verify dates are in ascending order by comparing strings
